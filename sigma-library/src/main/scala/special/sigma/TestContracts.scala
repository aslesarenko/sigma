package special.sigma {
  ...
  trait TestContracts extends Base { self: SigmaLibrary =>
    ...
    abstract class CrowdFundingContract(val deadline: Rep[Long], val minToRaise: Rep[Long], val backerPubKey: Rep[ProveDlog], val projectPubKey: Rep[ProveDlog]) extends CrowdFunding with DefaultContract;
