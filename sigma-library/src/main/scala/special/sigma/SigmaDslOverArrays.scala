package special.sigma {
  ...
  trait SigmaDslOverArrays extends Base { self: SigmaLibrary =>
    ...
    abstract class ContextOverArrays(val inputs: Rep[WArray[Box]], val outputs: Rep[WArray[Box]], val HEIGHT: Rep[Long]) extends Context {
    ...
    abstract class CrowdFundingContract(val timeout: Rep[Int], val minToRaise: Rep[Int], val backerPubKey: Rep[ProveDlog], val projectPubKey: Rep[ProveDlog]) extends CrowdFunding with DefaultContract;
