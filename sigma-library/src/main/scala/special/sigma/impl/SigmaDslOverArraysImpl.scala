...
!!!("Cannot convert from Box to TestBox: missing fields List(idBytes)")
    override def getDefaultRep = RTestBox(element[WArray[Byte]].defaultRepValue, 0l, element[Col[Byte]].defaultRepValue, element[Col[AnyValue]].defaultRepValue)
    override lazy val tag = {
      weakTypeTag[TestBox]
    }
  }
  ...
  class TestBoxIso
    ...
    override def to(p: Rep[(WArray[Byte], (Long, (Col[Byte], Col[AnyValue])))]) = {
      ...
      RTestBox(idBytes, value, propositionBytes, registers)
    }
...
!!!("Cannot convert from Context to TestContext: missing fields List(inputs, outputs, height, selfBox, vars)")
    override def getDefaultRep = RTestContext(element[WArray[Box]].defaultRepValue, element[WArray[Box]].defaultRepValue, 0l, element[Box].defaultRepValue, element[WArray[AnyValue]].defaultRepValue)
    override lazy val tag = {
  ...
  class TestContextIso
    ...
    override def to(p: Rep[(WArray[Box], (WArray[Box], (Long, (Box, WArray[AnyValue]))))]) = {
      ...
      RTestContext(inputs, outputs, height, selfBox, vars)
    }
