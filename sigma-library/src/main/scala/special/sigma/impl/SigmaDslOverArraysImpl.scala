...
trait SigmaDslOverArraysDefs extends scalan.Scalan with SigmaDslOverArrays {
  ...
  case class ContextOverArraysCtor
      (override val inputs: Rep[WArray[Box]], override val outputs: Rep[WArray[Box]], override val HEIGHT: Rep[Long], override val SELF: Rep[Box])
    extends ContextOverArrays(inputs, outputs, HEIGHT, SELF) with Def[ContextOverArrays] {
...
!!!("Cannot convert from Context to ContextOverArrays: missing fields List(inputs, outputs)")
    override def getDefaultRep = ContextOverArrays(element[WArray[Box]].defaultRepValue, element[WArray[Box]].defaultRepValue, 0l, element[Box].defaultRepValue)
    override lazy val tag = {
  ...
  type ContextOverArraysData = (WArray[Box], (WArray[Box], (Long, Box)))

  // 3) Iso for concrete class
  class ContextOverArraysIso
    extends EntityIso[ContextOverArraysData, ContextOverArrays] with Def[ContextOverArraysIso] {
    private lazy val _safeFrom = fun { p: Rep[ContextOverArrays] => (p.inputs, p.outputs, p.HEIGHT, p.SELF) }
    override def from(p: Rep[ContextOverArrays]) =
      tryConvert[ContextOverArrays, (WArray[Box], (WArray[Box], (Long, Box)))](eTo, eFrom, p, _safeFrom)
    override def to(p: Rep[(WArray[Box], (WArray[Box], (Long, Box)))]) = {
      val Pair(inputs, Pair(outputs, Pair(_HEIGHT, _SELF))) = p
      ContextOverArrays(inputs, outputs, _HEIGHT, _SELF)
    }
    lazy val eFrom = pairElement(element[WArray[Box]], pairElement(element[WArray[Box]], pairElement(element[Long], element[Box])))
  ...
  class ContextOverArraysCompanionCtor extends CompanionDef[ContextOverArraysCompanionCtor] with ContextOverArraysCompanion {
    ...
    def apply(inputs: Rep[WArray[Box]], outputs: Rep[WArray[Box]], HEIGHT: Rep[Long], SELF: Rep[Box]): Rep[ContextOverArrays] =
      mkContextOverArrays(inputs, outputs, HEIGHT, SELF)
...
!!!("Cannot convert from ProveDlog to ProveDlogEvidence: missing fields List(id)")
  ...
  def mkContextOverArrays
    (inputs: Rep[WArray[Box]], outputs: Rep[WArray[Box]], HEIGHT: Rep[Long], SELF: Rep[Box]): Rep[ContextOverArrays] = {
    new ContextOverArraysCtor(inputs, outputs, HEIGHT, SELF)
  }
  def unmkContextOverArrays(p: Rep[Context]) = p.elem.asInstanceOf[Elem[_]] match {
    case _: ContextOverArraysElem @unchecked =>
      Some((p.asRep[ContextOverArrays].inputs, p.asRep[ContextOverArrays].outputs, p.asRep[ContextOverArrays].HEIGHT, p.asRep[ContextOverArrays].SELF))
    case _ =>
      None
  }
